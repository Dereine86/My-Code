...
7.2.14: New Version with higher serial transmission rate for higher sampling rate.
*/
...
void loop() {
 ...
 if (DOFLOATS) {
   Serial.println(nHighHl + nLowHl);
   speedHl = FPGASensors::doFloatingCalculation(SPEEDHLTWOCYC, nHighHl + nLowHl);
   speedHr = FPGASensors::doFloatingCalculation(SPEEDHRTWOCYC, nHighHr + nLowHr);
 }
