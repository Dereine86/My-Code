...
float FPGASensors::doFloatingCalculation(const unsigned int type, const unsigned int data){
  switch (type) {
    case (ACCELERATION) : {
      float mg = data * 4;
      return (mg * 9.81) / 1000;
    ...
    case (SPEEDHR) : {
      if (data == 65535)
        ...
        return ((DELTAPHI * FPGACLOCKFREQUENCY * WHEELRADIUS) / (data * DCFHHR)) * 3.6;
    }
    case (SPEEDHL): {
      if (data == 65535)
        return 0.0f;
      else       
        return ((DELTAPHI * FPGACLOCKFREQUENCY * WHEELRADIUS) / (data * DCFHHL)) * 3.6;
    }
    case (SPEEDVL): {
      if (data == 65535)
        return 0.0f;
      else
        return ((DELTAPHI * FPGACLOCKFREQUENCY * WHEELRADIUS) / (data * DCFHVL)) * 3.6;
    }
    case (SPEEDVR): {
      if (data == 65535)
        return 0.0f;
      else
        return ((DELTAPHI * FPGACLOCKFREQUENCY * WHEELRADIUS) / (data * DCFHVR)) * 3.6;
    }  
  }
}
